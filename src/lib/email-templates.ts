/**
 * Pure HTML rendering for outgoing emails.
 *
 * Kept free of 'use server' / nodemailer so templates are unit-testable.
 * Every non-numeric interpolation MUST go through escapeHtml — several inputs
 * (budget names, AI-generated insight text) are user- or model-controlled.
 */
...
export function renderTestEmail(): { subject: string; html: string } {
    ...
    `);
    return { subject: 'YABA — Email Verified', html };
}
...
export interface BudgetAlertEmailData {
    ...
    spent: number;
    limit: number;
    tier: 'warning' | 'exceeded';
}

export function renderBudgetAlertEmail(data: BudgetAlertEmailData): { subject: string; html: string } {
    const { budgetName, category, spent, limit, tier } = data;
    const overBy = spent - limit;
    const pct = limit > 0 ? Math.round((spent / limit) * 100) : 0;
    const isWarning = tier === 'warning';

    const lastRow = isWarning
        ? `<tr>
                    <td style="border-top:1px solid rgba(255,255,255,0.06);${STYLES.rowLabel}">Remaining</td>
                    <td style="border-top:1px solid rgba(255,255,255,0.06);${STYLES.rowValue}color:#f59e0b;">$${(limit - spent).toFixed(2)} (${pct}% used)</td>
                </tr>`
        : `<tr>
    ...
    const html = wrapTemplate(`
        <h2 style="${STYLES.heading}">${isWarning ? 'Budget Warning — 80% Reached' : 'Budget Limit Reached'}</h2>
        <p style="${STYLES.subheading}">${escapeHtml(budgetName)} · ${escapeHtml(category)}</p>
        <div style="${STYLES.dataBox}">
            <table style="width:100%;border-collapse:collapse;">
                ...
                <tr>
                    ...
                    <td style="${STYLES.rowValue}color:${isWarning ? '#f59e0b' : '#ef4444'};">$${spent.toFixed(2)}</td>
                </tr>
                ${lastRow}
            </table>
        </div>
    ...
    const subject = isWarning
        ? `YABA — ${budgetName} at ${pct}% of budget`
        : `YABA — ${budgetName} Over Budget`;
    return { subject, html };
}
...
export function renderMonthlyReportEmail(data: MonthlyReportEmailData): { subject: string; html: string } {
    const categoriesHtml = data.topCategories
        .slice(0, 5)
        .map(
            (c) =>
                `<tr><td style="${STYLES.rowLabel}">${escapeHtml(c.name)}</td><td style="${STYLES.rowValue}">$${c.amount.toFixed(2)}</td></tr>`,
        )
        .join('');
    ...
    const insightsHtml = data.aiInsights
        .map(
            (i) =>
                `<div style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.04);"><strong style="color:#e0e0e0;font-size:13px;">${escapeHtml(i.title)}</strong><p style="color:#999;font-size:12px;margin:4px 0 0;line-height:1.5;">${escapeHtml(i.detail)}</p></div>`,
        )
        .join('');

    const html = wrapTemplate(`
        <h2 style="${STYLES.heading}">Monthly Financial Report</h2>
        <p style="${STYLES.subheading}">${escapeHtml(data.monthLabel)}</p>
    ...
    `);
    return { subject: `YABA — ${data.monthLabel} Report`, html };
}
